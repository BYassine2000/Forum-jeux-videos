							...
							<tbody>
								<?php if ($lesPaniers) { ?>
									<?php foreach ($lesPaniers as $unPanier) {
										$total = $total + $unPanier['prixpublication'] * $unPanier['quantite']; ?>
										<input type="hidden" name="numcommande" value="<?= $unPanier['numcommande']; ?>">
										<tr>
											<td><?= $unPanier['idpublication']; ?></td>
											<input type="hidden" name="idpublication" value="<?= $unPanier['idpublication']; ?>">
											<td><?= $unPanier['nompublication']; ?></td>
											<input type="hidden" name="nompublication" value="<?= $unPanier['nompublication']; ?>">
											<td>
												<?= number_format($unPanier['prixpublication'], 2, ',', ' '); ?> €
												<input type="hidden" class="iprice" value="<?= $unPanier['prixpublication']; ?>">
											</td>
											...
											<td>
												...
												<button type="button" class="btn btn-sm btn-outline-primary me-2" data-bs-toggle="modal" data-bs-target="#editQtePublication">
													<i class="bi bi-pencil-square"></i>
												</button>
												<a href="panier?action=sup&numcommande=<?= $unPanier['numcommande']; ?>&idpublication=<?= $unPanier['idpublication']; ?>&idclient=<?= $unPanier['idclient']; ?>" class="btn btn-sm btn-outline-danger">
...
<div class="modal fade" id="editQtePublication" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1" aria-hidden="true">
  	<div class="modal-dialog">
    	<div class="modal-content">
      		<div class="modal-header">
        		<h5 class="modal-title">Modifier la quantité d'un publication</h5>
        		<button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
      		</div>
      		<form method="post" action="">
      			<div class="modal-body">
	        		<div class="mb-3">
	        			<select name="idpublication" class="form-select form-control">
	        				<?php foreach ($lesPaniers as $unPublication) { ?>
	        				<option value="<?= $unPublication['idpublication']; ?>">
	        					<?= $unPublication['nompublication']; ?>
	        				</option>
	        				<?php } ?>
	        			</select>
